...
function validateAcceptanceChecks(acceptanceChecks, checks) {
  if (!Array.isArray(acceptanceChecks) || acceptanceChecks.length === 0) {
    checks.push(check('proposal.acceptance_checks', 'warning', false, 'acceptance_checks should reference an upstream gate or a regression check'));
    return;
  }
...
Read-only owner gate for pamem-owned memory handoff artifacts. It validates
that the artifact targets pamem, preserves the proposal-only boundary, contains
compact source references, and embeds no transcripts or raw logs. It does not
apply, rewrite, propagate, or stage memory changes.

Examples:
  pamem check proposals/example__01__memory_proposal.json --workspace <workspace> --json`);
}
