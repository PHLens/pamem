...
export function onboardPamem(args, context) {
  ...
  if (parsed.syncExecutor) {
    setTomlValue(configFile, 'sync', 'executor', tomlString(parsed.syncExecutor));
  }
...
export function onboardUsage() {
  console.log(`Usage: pamem onboard <workspace> [--agent-home] [--profile <onboarding|coder|reviewer|researcher|wiki>] [--runtime <cli|slock>] [--agent-id <id>] [--memory-repo <path>] [--sync-remote <target>] [--sync-ref <ref>] [--sync-executor <name>] [--git-author-name <name>] [--git-author-email <email>] [--force]
    ...
    if (parsedOption) {
      ...
      switch (parsedOption.name) {
        ...
        case '--sync-ref':
          ...
          break;
        case '--sync-executor':
          parsed.syncExecutor = parsedOption.value;
          break;
...
function parseValueOption(arg, args, index) {
  const names = ['--profile', '--runtime', '--agent-id', '--memory-repo', '--sync-remote', '--sync-ref', '--sync-executor', '--git-author-name', '--git-author-email'];
