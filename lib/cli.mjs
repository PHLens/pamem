...
export function main(argv = process.argv.slice(2)) {
  ...
  switch (command) {
    ...
    case 'hook-json':
    case 'context':
      runBash(join(scriptsDir, 'pamem-cli.sh'), [command, ...rewriteWorkspaceArgs(args)]);
      break;
    case 'lint':
...
function launchUsage() {
  console.log(`Usage: pamem launch --role <role> [--runtime cli|slock] [--agent-id <id>] [--workspace <path>] [--resume] [--print-env] [-- <command> [args...]]
...
function parseLaunchArgs(args) {
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    if (['--memory-repo', '--sync-remote', '--sync-ref', '--sync-executor'].includes(arg)) {
      if (i + 1 >= args.length) fail(`missing value for ${arg}`);
      parsed.onboardArgs.push(arg, args[++i]);
      continue;
    }
    if (/^--(memory-repo|sync-remote|sync-ref|sync-executor)=/.test(arg)) {
...
function runLaunch(args) {
  ...
  if (parsed.runtime === 'cli') {
    ...
    if (parsed.launchArgs.length > 0) cliArgs.push('--', ...parsed.launchArgs);

    runBash(join(scriptsDir, 'pamem-cli.sh'), [parsed.resume ? 'resume' : 'start', ...cliArgs]);
  }
  ...
  runBash(join(scriptsDir, 'pamem-cli.sh'), ['status', '--workspace', parsed.workspace]);
}

function rewriteWorkspaceArgs(args) {
  const rewritten = [];
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    if (arg === '--workspace') {
      if (i + 1 >= args.length) fail('missing value for --workspace');
      sawWorkspace = true;
      rewritten.push('--workspace', args[++i]);
      continue;
    }
    if (arg.startsWith('--workspace=')) {
      sawWorkspace = true;
      rewritten.push('--workspace', arg.slice('--workspace='.length));
      continue;
    }
    if (arg === '--agent-id') {
      if (i + 1 >= args.length) fail('missing value for --agent-id');
      agentId = args[++i];
      rewritten.push('--agent-id', agentId);
      continue;
    }
    if (arg.startsWith('--agent-id=')) {
  ...
  if (!sawWorkspace) {
    ...
    if (workspace) rewritten.unshift('--workspace', workspace);
  }
