...
function launchUsage() {
  console.log(`Usage: pamem launch --role <role> [--runtime codex|claude|cli|slock] [--runtime-arg <arg>] [--agent-id <id>] [--workspace <path>] [--memory-repo <path>] [--sync-remote <target>] [--sync-ref <ref>] [--git-author-name <name>] [--git-author-email <email>] [--resume] [--print-env] [-- <command> [args...]]
...
Use --runtime codex or --runtime claude to select the CLI launcher while keeping
pamem launch options such as --memory-repo parsed by pamem. Use --runtime-arg to
pass launcher-specific arguments. The legacy '-- <command> [args...]' form is
still accepted for custom launchers and compatibility.
...
function parseLaunchArgs(args) {
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    if (arg === '--runtime-arg') {
      if (i + 1 >= args.length) fail('missing value for --runtime-arg');
      parsed.launchArgs.push(args[++i]);
      continue;
    }
    if (arg.startsWith('--runtime-arg=')) {
      parsed.launchArgs.push(arg.slice('--runtime-arg='.length));
      continue;
    }
    if (arg === '-h' || arg === '--help') {
      launchUsage();
...
function normalizeLaunchRuntime(parsed) {
  if (parsed.runtime === 'codex' || parsed.runtime === 'claude') {
    if (parsed.launchArgs.length > 0 && parsed.launchArgs[0] !== parsed.runtime) {
      parsed.launchArgs = [parsed.runtime, ...parsed.launchArgs];
    } else if (parsed.launchArgs.length === 0) {
      parsed.launchArgs = [parsed.runtime];
    }
    parsed.launcher = parsed.runtime;
    parsed.runtimeMode = 'cli';
    return;
  }

  if (parsed.runtime === 'cli' || parsed.runtime === 'slock') {
    parsed.runtimeMode = parsed.runtime;
    parsed.launcher = parsed.launchArgs[0] || '';
    return;
  }
...
function runLaunch(args) {
  ...
  normalizeLaunchRuntime(parsed);

  if (!parsed.role) fail('pamem launch requires --role');
  requireSupportedRole(parsed.role);

  if (parsed.runtimeMode === 'cli') {
    if (!parsed.workspace) {
      if (!parsed.agentId) {
        fail(`pamem launch --runtime ${parsed.runtime} requires --agent-id when --workspace is not provided`);
      }
  ...
  switch (command) {
    case 'start':
      ...
      if (state.launchArgs.length > 0) {
        const launchArgs = runtimeLaunchArgs(state.launchArgs);
        const session = recordSession(state, 'start', launchArgs);
        runAndExit(launchArgs[0], launchArgs.slice(1), { cwd: state.workspace, env: launchEnv(state, 'start', session) });
      }
      printStatus(state);
    ...
    case 'resume':
      ...
      if (state.launchArgs.length > 0) {
        const launchArgs = runtimeLaunchArgs(state.launchArgs);
        const session = recordSession(state, 'resume', launchArgs);
        runAndExit(launchArgs[0], launchArgs.slice(1), { cwd: state.workspace, env: launchEnv(state, 'resume', session) });
      }
      if (state.printEnv) {
        printStatus(state);
      ...
      }
      {
        ...
        if (argsToResume.length === 0) {
          ...
          console.error(`Run 'pamem launch --role <role> --agent-id ${state.agentId} --runtime codex|claude' first, configure [runtime.resume].command, or pass an explicit resume command after --.`);
          process.exit(1);
        }
        const launchArgs = runtimeLaunchArgs(argsToResume);
        const session = recordSession(state, 'resume', launchArgs);
        runAndExit(launchArgs[0], launchArgs.slice(1), { cwd: state.workspace, env: launchEnv(state, 'resume', session) });
      }
      break;
