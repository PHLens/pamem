...
import { runSkillCommand } from './skills.mjs';
...
Commands:
  ...
  skill                  Inspect read-only agent skill visibility.
...
List configured CLI agent homes under the XDG pamem agents directory. Use
...
function runList(args) {
  ...
  const agentsDir = join(dataHome(), 'pamem', 'agents');
  const agents = [];
  if (existsSync(agentsDir)) {
    for (const name of readdirSync(agentsDir).sort()) {
      ...
      if (!hasConfig(resolved)) continue;
      agents.push({
        agent_id: configuredAgentId(resolved),
        role: defaultProfile(resolved),
        runtime: runtimeMode(resolved),
        home: resolved,
  ...
  if (json) {
    console.log(JSON.stringify({ agents_dir: agentsDir, agents }, null, 2));
    return;
  }

  if (agents.length === 0) {
    console.log(`No pamem agents found under ${agentsDir}`);
    return;
  }
  console.log('agent_id\truntime\trole\thome');
  for (const agent of agents) {
    console.log(`${agent.agent_id}\t${agent.runtime}\t${agent.role}\t${agent.home}`);
  }
}
