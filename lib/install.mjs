...
import { configPath, memoryRepoGitAuthor, memoryRepoRoot, memoryRepoSyncRemote, runtimeMode } from './config.mjs';
...
const roles = {
  ...
  researcher: {
    ...
    experience: 'research findings, corrections, and retrieval lessons',
  },
  wiki: {
    title: 'Wiki',
    workflow: 'curation workflow',
    experience: 'wiki workflow, curation, retrieval, and handoff lessons',
  },
};
...
export function installPamem(args, context) {
  ...
  ensureMemoryRepoSkeleton(repoRoot, context.assetsDir);
  ...
  if (!agentHomeMode) {
    if (mode === 'cli') {
      ...
      copyIfMissing(join(context.assetsDir, 'notes', 'experience.md.template'), join(paths.notesDir, 'experience.md'));
      if (!nonEmptyFile(paths.memoryPath)) {
        cpSync(join(context.assetsDir, 'MEMORY.md.template'), paths.memoryPath);
      }
...
function ensureSkillLink(src, dst) {
  ...
  if (existsSync(dst) && !isSymlink(dst)) {
    console.error(`pamem skill target exists and is not a symlink: ${dst}`);
...
function ensureSlockWorkspaceMemory(file, assetsDir) {
  const template = readFileSync(join(assetsDir, 'slock', 'MEMORY.md.template'), 'utf8');
...
function ensureMemoryRepoSkeleton(repoRoot, assetsDir) {
  for (const dir of ['governance', 'shared', 'roles', 'projects', 'archive']) {
    mkdirSync(join(repoRoot, dir), { recursive: true });
  }

  copyIfMissing(join(assetsDir, 'MEMORY.md.template'), join(repoRoot, 'MEMORY.md'));
...
function printMemoryRepoInitReminder(repoRoot, workspace) {
  ...
  console.log('Then rerun pamem repair, launch, or onboard so pamem can apply the config.');
}
...
function ensureCodexConfig(file) {
  ...
  if (lines.some((line) => /^[ \t]*codex_hooks[ \t]*=/.test(line))) {
    writeFileSync(file, lines.map((line) => (/^[ \t]*codex_hooks[ \t]*=/.test(line) ? 'codex_hooks = true' : line)).join('\n'));
    return;
  }
...
function removeCodexSkillLinks(repoRoot, codexSkillsDir) {
  const skillsDir = join(repoRoot, 'skills');
  if (!existsSync(skillsDir) || !existsSync(codexSkillsDir)) return;
