...
export function resolutionObject(state) {
  const result = {
    status: 'ok',
...
export function printResolve(state) {
  const result = resolutionObject(state);
  if (state.json) {
...
function parseRuntimeArgs(args, command) {
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    if (arg === '--json') {
      if (command !== 'resolve') failArg('--json is only supported for pamem resolve');
      parsed.json = true;
      continue;
    }
    if (arg === '-h' || arg === '--help') {
...
function runtimeUsage(command) {
  if (command === 'resolve') {
    console.log('Usage: pamem resolve [--workspace <path>] [--agent-id <id>] [--json]');
    return;
  }
