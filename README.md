...
- `scripts/memory-lint.sh`: read-only lint for the workspace-local memory config and shared memory repo
...
`scripts/memory-lint.sh` reads the workspace-local `.pamem/config.toml` and lints the configured memory repo. It does not require a `.pamem/config.toml` inside the memory repo itself.
